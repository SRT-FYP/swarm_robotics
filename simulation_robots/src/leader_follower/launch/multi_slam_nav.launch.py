...
from launch.actions import IncludeLaunchDescription, LogInfo, GroupAction, AppendEnvironmentVariable, OpaqueFunction, ExecuteProcess, RegisterEventHandler
...
def generate_launch_description():
    ...
    tb3_house = os.path.join(
                package_dir,
                'worlds',
                'turtlebot3_house_closed.world.xacro'
            )
    world = tb3_house
    ...
    gazebo_param_file = os.path.join(package_dir, 'params', 'gazebo_params.yaml')
    ...
    gazebo_server = IncludeLaunchDescription(
        ...
        launch_arguments={'gz_args': ['-r -s ', world_sdf],
                          'extra_gazebo_args': '--ros-args --params-file ' + gazebo_param_file}.items()
    )
    ...
    launch_map_expansion = IncludeLaunchDescription(
            PythonLaunchDescriptionSource(os.path.join(map_merge_pkg, 'launch', 'map_expansion_modular.launch.py')),
        )
